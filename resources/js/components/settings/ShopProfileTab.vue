...
const switchSubtab = (subtab: string) => {
    router.get('/settings/profile', { tab: 'shop', subtab }, { preserveState: false });
};
        ...
        <div class="space-y-6">
            ...
            <div class="flex gap-1 border-b">
                <button
                    ...
                    :class="subtab === 'profile' ? 'border-primary text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground'"
                    @click="switchSubtab('profile')"
                >
                    Profile
                </button>
                <button
                    class="px-4 py-2 text-sm font-medium transition-colors border-b-2 -mb-px"
                    :class="subtab === 'locations' ? 'border-primary text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground'"
                    @click="switchSubtab('locations')"
                >
                    Locations
                    <span v-if="shopLocations.length" class="ml-1.5 rounded-full bg-muted px-1.5 py-0.5 text-xs">{{ shopLocations.length }}</span>
                </button>
            </div>
            ...
            <!-- LOCATIONS SUBTAB -->
            <template v-if="subtab === 'locations'">
                ...
                <!-- Locations list -->
                <div v-if="shopLocations.length > 0" class="space-y-4">
                    <Card v-for="location in shopLocations" :key="location.id">
                        <CardHeader>
                            <div class="flex items-start justify-between gap-4">
